...
class Engine
{
    ...
    {
        $timezone = Configuracoes::get('DEFAULT_TIME_ZONE');
    ...
    public function setContainer(
        ...
        if (!is_int($position)) {
            throw new AppException(
                 'A posição "' . $position . '" do container "' 
                 . $chave . '" não é válida', 
                 1009,
                 403
            );
    ...
    public function initContainers()
    {
        ...
        if (!file_exists($pastaAbir)) {
            throw new AppException(
                'A pasta de containers não foi encontrada no caminho: ' 
                . $pastaAbir, 
                1010, 
                500
            );
        ...
        if (!$abrirPasta) {
            throw new AppException(
                'Não foi possível abrir a pasta de containers. ' 
                . var_dump($abrirPasta, true), 
                1011, 
                500
            );
        }
        ...
        $this->setContainer(
            'settings', 
            Configuracoes::get('SLIM_SETTINGS'), 
            0
        );
    }
    ...
    public function saveArquivo(
        ...
        $pasta_salvar = $criarPasta 
            ? $this->criarPasta($pasta_arquivo) . $arquivo_salvar 
            : $pasta_arquivo;
