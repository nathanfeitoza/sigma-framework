...
'.$content.'
        ...
        protected function _tag_open_IMG($param)
        {
            ...
            $src    = str_replace('-esp!@', ' ', $param['src']);
            
            @$documento_explode = explode('/',__DIR__); // Aqui pegamos o script atual em execução e recuperamos seu fim
            $documento_atual = end($documento_explode);
            $root_document = str_replace($documento_atual, '', __DIR__); 

            $new_root = explode("/", $root_document);
            $new_root = array_filter($new_root);

            $root_get = end($new_root);
            $root_atual = implode("/",str_replace($root_get, '', $new_root));
            
            //$src = '/'.$root_atual.'/Requests/'.$src;
